alter table public.events
  drop constraint if exists events_target_score_check;

alter table public.events
  add constraint events_target_score_check check (target_score between 0 and 1000);
...
begin
  ...
  if event_state not in ('DRAFT','READY') then
    raise exception 'Il punteggio non è modificabile dopo l’avvio';
  end if;
